...
class SubscriptionController extends Controller
{
	...
	public function unitpay(User $user)
	{
        ...
        $payment_amount = $user->profile->finalPrice;

        $user_email = "email@paypal.com";

        $item_name = $user->profile->username;
        ...
        $payment_no = $subscrId;
        $item_name = 'Благотворительное пожертвование @'.$user->profile->username;

        return UnitPay::generatePaymentForm($payment_amount, $payment_no, $user_email, $item_name, $currency);

		//return view('subscribe.unitpay', compact('payment_fields'));
	}

    // unitpay route
    public function payPayeer(User $user)
    {
        if (auth()->id() == $user->id) {
            alert()->info(__('general.dontSubscribeToYourself'));
            return back();
        ...
        $m_amount = number_format($payment_amount, 2, '.', '');
        $m_curr = 'RUB';
        $m_desc = base64_encode('Благотворительное пожертвование @'.$user->profile->username);
        ...
        $subPlan->creator_id = $user->id;
        $subPlan->subscriber_id = auth()->user()->id;
        $subPlan->subscription_id = $m_orderid;
        $subPlan->gateway = 'UnitPay';
        $subPlan->subscription_date = now();
        $subPlan->subscription_expires = now();
        $subPlan->subscription_price = $price;
