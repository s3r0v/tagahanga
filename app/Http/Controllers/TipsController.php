...
class TipsController extends Controller
{
    ...
    public function processTip(Post $post, Request $r)
    {
        ...
        $this->validate(
            $r,
            [
                'gateway' => 'required|in:Card,PayPal,unitPay',
        ...
        } elseif ($r->gateway == 'unitPay') {
            return $this->unitPayTip($post, $creator, $tipper, $amount);
        }
    }
    ...
    //TODO
    public function unitPayTip($post, $creator, $tipper, $amount) {
        ...
        try {
            ...
            session(['unitPayTipId' => $tip->id]);
            
            $currency =  'RUB';
            $payment_no = 'tip'.$tip->id;

            $creator = User::findOrFail($creator->id);
            $item_name = 'Благотворительное пожертвование @'.$creator->profile->username;
            $user_email = "email@paypal.com";

            return UnitPay::generatePaymentForm($amount, $payment_no, $user_email, $item_name, $currency);
